...
api:
	@printf "$(OK_COLOR)==== Launch fastapi page ====$(NO_COLOR)\n"
	$(UVICORN) api:app
...
help:
	...
	@echo -e "$(WARN_COLOR)- make api			        : Launch fastapi page"
...
generate:
	@printf "$(OK_COLOR)==== Genetate new gRPC interfaces ====$(NO_COLOR)\n"
	@$(eval args := $(words $(filter-out --,$(MAKECMDGOALS))))
	@if [ "${args}" -eq 2 ]; then \
		echo "$(OK_COLOR) Generate gRPC interfaces files ${APPNAME}$(NO_COLOR)\n"; \
		$(PYTHON) -m grpc_tools.protoc -I. --python_out=. --grpc_python_out=. protos/${APPNAME}.proto; \
	elif [ "${args}" -gt 2 ]; then \
		echo "$(ERROR_COLOR)The proto file name must not contain spaces!$(NO_COLOR)\n"; \
	else \
		echo "$(ERROR_COLOR)Enter the proto file name!$(NO_COLOR)\n"; \
	fi
